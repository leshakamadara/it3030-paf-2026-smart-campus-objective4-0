...
export default function CreateTicket({
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-6">
        <div className="w-16 h-16 bg-[#edfaf2] border border-[#b3e8c4] rounded-full flex items-center justify-center text-3xl mb-4">
          ✓
        </div>
        <h2 className="text-xl font-[590] text-[#191a1b] mb-2">Ticket Submitted!</h2>
        <p className="text-[#62666d] text-sm max-w-sm mb-6">
          Your incident report has been logged. You'll receive updates as the team works on it.
        </p>
        <button
          onClick={onCancel}
          className="px-6 py-2.5 bg-[#5e6ad2] text-white text-xs font-[510] rounded-md hover:bg-[#7170ff] transition-colors"
        >
          Back to My Tickets
        </button>
      </div>
  ...
  return (
    <div className="max-w-xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-2xl font-[590] tracking-[-0.44px] text-[#191a1b]">Report an Incident</h1>
        <p className="text-[#62666d] text-sm mt-1">Fill in the details below and our maintenance team will be notified.</p>
      </div>
        ...
        {steps.map((s, i) => (
          <div key={s} className="flex items-center gap-1 flex-1">
            <div className="flex flex-col items-center flex-1">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-[590] transition-all ${
                i < step ? "bg-[#5e6ad2] text-white" :
                i === step ? "bg-[#5e6ad2] text-white ring-4 ring-[#5e6ad2]/20" :
                "bg-[#f3f4f5] text-[#8a8f98] border border-[#d0d6e0]"
              }`}>
                {i < step ? "✓" : i + 1}
              </div>
              <span className={`text-[10px] mt-1 font-[510] whitespace-nowrap hidden sm:block ${
                i === step ? "text-[#5e6ad2]" : i < step ? "text-[#7170ff]" : "text-[#8a8f98]"
              }`}>
                {s}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={`h-0.5 flex-1 mb-4 ${i < step ? "bg-[#5e6ad2]" : "bg-[#d0d6e0]"}`} />
            )}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div>
          <h2 className="text-base font-[590] text-[#191a1b] mb-4">What type of issue is this?</h2>
              ...
              <button
                ...
                className={`flex items-center gap-3 p-4 rounded-xl border-2 text-left transition-all ${
                  form.category === c.value
                    ? "border-[#5e6ad2] bg-[#eef0fb] text-[#5e6ad2]"
                    : "border-[#d0d6e0] bg-white text-[#191a1b] hover:border-[#5e6ad2]/40"
                }`}
              >
                <span className="text-xl">{c.icon}</span>
                <span className="text-sm font-[510] leading-tight">{c.value}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="space-y-5">
          <h2 className="text-base font-[590] text-[#191a1b]">Describe the incident</h2>

          <div>
            <label className="block text-xs font-[510] text-[#62666d] mb-1.5 uppercase tracking-wide">
              Incident Title <span className="text-red-400">*</span>
            </label>
            <input
              className="w-full border border-[#d0d6e0] rounded-md px-4 py-3 text-sm text-[#191a1b] bg-white focus:outline-none focus:ring-2 focus:ring-[#5e6ad2]/40 placeholder:text-[#8a8f98]"
          ...
          <div>
            ...
            <textarea
              rows={4}
              className="w-full border border-[#d0d6e0] rounded-md px-4 py-3 text-sm text-[#191a1b] bg-white focus:outline-none focus:ring-2 focus:ring-[#5e6ad2]/40 resize-none placeholder:text-[#8a8f98]"
          ...
          <div>
            <label className="block text-xs font-[510] text-[#62666d] mb-2 uppercase tracking-wide">
              Priority
            </label>
                  ...
                  <button
                    ...
                    className={`flex items-start gap-3 p-3 rounded-xl border-2 text-left transition-all ${
                      form.priority === p
                        ...
                        : "border-[#d0d6e0] text-[#62666d] hover:border-[#5e6ad2]/40"
                    }`}
                  >
                    <span className={`w-2 h-2 rounded-full mt-0.5 shrink-0 ${m.dot}`} />
                    <div>
                      <p className="text-xs font-[590]">{m.label}</p>
      ...
      {/* Navigation */}
      <div className="flex items-center justify-between mt-8 pt-5 border-t border-[#d0d6e0]">
        <button
          onClick={() => step > 0 ? setStep(step - 1) : onCancel()}
          className="px-4 py-2.5 text-sm text-[#62666d] hover:text-[#191a1b] font-[510] transition-colors"
        >
          {step > 0 ? "← Back" : "Cancel"}
        </button>
        <div className="flex items-center gap-2">
          <span className="text-xs text-[#8a8f98]">{step + 1} / {steps.length}</span>
          {step < 3 ? (
            <button
              onClick={() => setStep(step + 1)}
              disabled={!canAdvance}
              className="px-6 py-2.5 bg-[#5e6ad2] text-white text-xs font-[510] rounded-md hover:bg-[#7170ff] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Continue →
            </button>
          ) : (
            <button
              ...
              className="px-6 py-2.5 bg-[#27a644] text-white text-xs font-[510] rounded-md hover:bg-[#1e8a35] transition-colors"
            >
              Submit Ticket ✓
            </button>
          )}
